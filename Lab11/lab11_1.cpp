...
public:
    ...
    {
        if (!opened)
            throw runtime_error(name + ": not open");
...
public:
    DiskDevice(string n, size_t sectors_) : IODevice(move(n)), sectors(sectors_, string(SECTOR, '\0')),

                                            totalSectors(sectors_)
    {
    }
    bool open() override
    {
        opened = true;
        cout << "[DISK] " << name << " spun up\n";
        return true;
    }
    void close() override
    {
        opened = false;
        cout << "[DISK] " << name << " parked\n";
    }
...
public:
    NetworkDevice(string n, string ip_, int port_) : IODevice(move(n)), ip(move(ip_)), port(port_) {}
    bool open() override
    {
        opened = true;
        cout << "[NET] " << name << " socket opened\n";
        return true;
    }
    void close() override
    {
        opened = connected = false;
        cout << "[NET] " << name << " disconnected\n";
    }
    string read(size_t n) override
    {
        if (!opened)
    ...
    bool write(const string &data) override
    {
        if (!opened)
            return false;
        cout << "[NET] sent " << data.size() << " bytes to "
             << ip << ":" << port << "\n";
        return true;
    }
    string getInfo() const override
...
public:
    ...
    shared_ptr<IODevice> get(const string &n)
    {
        ...
        if (it == table.end())
            throw runtime_error("Device not found: " + n);
        return it->second;
    }
    void listAll() const
    {
